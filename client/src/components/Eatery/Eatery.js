...
const Eatery = (props) => {
    ...
    useEffect(() => {
      ...
      if (eateryDetails) {
        ...
        // static image src
        const source = `https://maps.googleapis.com/maps/api/staticmap?center=${coordinates.latitude},${coordinates.longitude}` +
                        '&zoom=17' +
                        '&size=300x300' +
                        '&scale=2' +
                        '&maptype=roadmap' +
      ...
      const renderTooltip = (props) => (
        ...
        return (
          ...
          <h1 className="eatery-title fw-bold">
            {name}
          </h1>

          <div>
          {categories.map((category) =>(
            <Badge bg="primary" text="dark" className="me-2 my-2 categories shadow">
            {category.title}
            </Badge>
          ))}
          </div>
          ...
          {price ? <Badge bg="light" text="dark" className="budget shadow my-2 border"> {price}  </Badge> : <div> </div> }
          <OverlayTrigger
          placement="right"
          ...
          <Badge bg="light" text="dark" className="rating shadow my-2 border"> 
            <div className="font-style fs-4">
              Rating: {rating}
              <Star size={24} color="#fae500" weight="fill" className="ms-1" />
            </div>
          </Badge>
          </OverlayTrigger>
          </div>
          ...
          <div className="opening-and-googleimage d-flex justify-content-between">
            <Card style={{ width: '18rem', height: '10rem' }}>
              <Card.Body>
                <Card.Title 
                className="fw-bold font-style fs-4 text-center text-decoration-underline d-flex"> 
                <div className="me-3"> <Clock size={32} color="#00bbfa" weight="duotone" /> </div>
                <div> Opening Hours </div> 
                </Card.Title>
              </Card.Body>

              <ListGroup variant="flush">
              {hours !== undefined ? hours[0]?.open?.slice(0, 7).map((day, index) =>(
              <ListGroup.Item>
